...
class DBManager{
    ...
    public function getUserTblByid($kensaku_word,$hospital_map,$hospital_name,$hospital_address,$hospital_phone_number,$medical_specialty){
        $pdo = $this->dbConnect();
        $sql = "SELECT * FROM hosapo_hospital_tbl WHERE hospital_name LIKE ? ";
        $ps = $pdo->prepare($sql);
        $ps->bindValue(1,"%".$kensaku_word."%",PDO::PARAM_STR);
        $ps->bindValue(2,"%".$hospital_map."%",PDO::PARAM_STR);
        $ps->bindValue(3,"%".$hospital_name."%",PDO::PARAM_STR);
        $ps->bindValue(4,"%".$hospital_address."%",PDO::PARAM_STR);
        $ps->bindValue(5,"%".$hospital_phone_number."%",PDO::PARAM_INT);
        $ps->bindValue(6,"%".$medical_specialty."%",PDO::PARAM_STR);
